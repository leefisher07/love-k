...
// GET /api/admin/promo-codes - 获取验证码列表
  ...
  try {
    ...
    return NextResponse.json({
      promoCodes: promoCodes.map(pc => ({
        ...
        batchNote: pc.batchNote,
        createdAt: pc.createdAt.toISOString(),
        firstUsedAt: pc.firstUsedAt?.toISOString(),
        lastUsedAt: pc.lastUsedAt?.toISOString(),
        hasReports: pc._count.reports > 0,
      })),
    })
  } catch (error) {
